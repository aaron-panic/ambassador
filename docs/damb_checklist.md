...
- [ ] `struct AtlasRuntime`
  ...
  - [ ] Notes:
    - [ ] No image reference here (image is held by layer runtime context)

- [ ] `struct MapCellRuntime`
  - [ ] Data members:
    - [ ] atlas record handle/index (recommended compact integer)
  - [ ] Notes:
    - [ ] No per-cell pointers

- [ ] `struct MapRuntime`
  - [ ] Data members:
    - [ ] `u32 width`
    - [ ] `u32 height`
    - [ ] cell size (tile width/height or one scalar if square)
    - [ ] `std::vector<MapCellRuntime> cells` (row-major flattened)
