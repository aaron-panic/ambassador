...
namespace amb {
    ...
    class Dambassador {
      ...
      private:
        ...
        ManifestSpec parseManifest(const std::filesystem::path& manifest_path) const;
        ...
        ChunkBlob buildImageChunk(const ManifestSpec& manifest, const std::filesystem::path& base_dir) const;
        ChunkBlob buildAtlasChunk(const ManifestSpec& manifest) const;
        ChunkBlob buildMapChunk(const ManifestSpec& manifest) const;

        void writeDamb(const ManifestSpec& manifest, const std::filesystem::path& manifest_path) const;
    };
}
